...
# input controls
ticker = TextInput(title="Ticker to plot (Yahoo name)", value='AAPL')
...
p = Figure(plot_height=600, plot_width=800, title="Moving Average Positions", x_axis_type='datetime', tools=tools)
p.line(x='date', y='price', alpha=0.3, color='Black', line_width=2, source=source, legend='Close')
...
def update(attrname, old, new):
    ...
    source.data = dict(
            date = df.index,
            price = df['Close'],
            price_tool = df['Close'].apply(lambda x: '%.2f' % x),
    ...
    df['pos'] = df['position'].map({-1:'Sell',1:'Buy'})
    ...
    buy.data = dict(
            ...
            price_tool = df.loc[buy_idx, 'Close'].apply(lambda x: '%.2f' % x),
    ...
    sell.data = dict(
            ...
            price_tool = df.loc[sell_idx,'Close'].apply(lambda x: '%.2f' % x),
