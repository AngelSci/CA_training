...
from bokeh.models.widgets import # widgets
...
## Step 2: Tools
# create a hover tool that shows
#   the school name (@name)
#   the cost (@cost)
#   the admission rate (@adm)
## Add other tools

## Step 3: Widgets
# Add a slider to set the maximum admission rate
# Add a text box to input the desired state
...
def update(attrname, old, new):
    ...
    ## Step 4: update column data sources
    idx = filter_by(...) # use 'ADM_RATE' to filter by maximum admission rate
    st = by_state(...) # use 'STABBR' to filter by state abbreviation (capital)
...
## Step 5: register the controls
controls = # provide a list of widgets
