...
def random_array():
    if os.path.exists(os.path.join(data_path, 'random.hdf5')):
        return

    print("Create random data for array exercise")

    with h5py.File(os.path.join(data_path, 'random.hdf5')) as f:
...
def accounts_csvs(num_files, n, k):
    fn = os.path.join(data_path, 'accounts.%d.csv' % (num_files - 1))
    ...
    for i in range(num_files):
        df = account_entries(n, *args)
        df.to_csv(os.path.join(data_path, 'accounts.%d.csv' % i),
                  index=False)


def accounts_json(num_files, n, k):
    fn = os.path.join(data_path, 'accounts.%02d.json.gz' % (num_files - 1))
    if os.path.exists(fn):
        return
    ...
    for i in range(num_files):
        ...
        fn = os.path.join(data_path, 'accounts.%02d.json.gz' % i)
        with gzip.open(fn, 'wb') as f:
            f.write(os.linesep.join(map(json.dumps, seq)).encode())


def create_weather(growth=3200):
    filenames = sorted(glob(os.path.join(data_path, 'weather-small', '*.hdf5')))

    if not os.path.exists(os.path.join(data_path, 'weather-big')):
        os.mkdir(os.path.join(data_path, 'weather-big'))
    ...
    for fn in filenames:
        ...
        out_fn = os.path.join(data_path, 'weather-big', os.path.split(fn)[-1])
